    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center" role="dialog" aria-modal="true">
      ...
      <div className="relative w-full sm:w-[420px] sm:mb-8 sm:rounded-[24px] bg-paper rounded-t-[26px] shadow-sheet sheet-in max-h-[88vh] overflow-y-auto">
        <div className="px-[26px] pt-4 pb-3 flex items-center justify-between">
          <span className="font-serif text-[1.5rem] text-ink">Ir a</span>
        ...
        <div className="pb-[18px] pl-[26px]">
          <div className="text-[0.6875rem] font-bold tracking-[.14em] text-ink-faint mb-3 uppercase">
          ...
          <div className="flex gap-2 overflow-x-auto pr-[26px]">
            {strip.map((iso) => {
              ...
              return (
                ...
                >
                  <div className={`text-[0.625rem] font-bold ${isSel ? 'text-ink-onDarkFaint' : 'text-ink-faintest'}`}>
                    {weekdayShort(iso)}
                  </div>
                  <div className={`font-serif text-[1.375rem] ${isSel ? 'text-ink-onDark' : 'text-ink-faint'}`}>
                    {d.getDate()}
                  </div>
                </button>
              )
        ...
        <div className="px-[26px] pt-[18px] border-t border-paper-divider">
          ...
          <div className="grid grid-cols-7 gap-1 text-center text-[0.625rem] font-bold text-ink-faintest mb-1.5">
            {['L', 'M', 'M', 'J', 'V', 'S', 'D'].map((d, i) => (
              <div key={i}>{d}</div>
            ))}
          </div>

          <div className="grid grid-cols-7 gap-1 font-serif text-[1.0625rem] text-ink text-center pb-2">
          ...
          <div className="flex gap-2 mt-4 pb-6">
            <button
              onClick={() => pick(today)}
              className="flex-1 text-center text-[0.8125rem] font-semibold text-azul border border-azul-tintBorder rounded-full py-[9px]"
            >
              Hoy
            </button>
            <button
              onClick={() => pick(tomorrow)}
              className="flex-1 text-center text-[0.8125rem] font-semibold text-ink-soft border border-paper-line rounded-full py-[9px]"
            >
              Mañana
            </button>
            <button
              onClick={() => pick(next.iso)}
              className="flex-1 text-center text-[0.8125rem] font-semibold text-ink-soft border border-paper-line rounded-full py-[9px] capitalize"
            >
              {next.label}
            </button>
          </div>
