...
function Checkbox({ done, overdue, onToggle }: { done: boolean; overdue: boolean; onToggle: () => void }) {
  return (
    <button
      ...
      className={[
        'w-[20px] h-[20px] rounded-[5px] flex items-center justify-center shrink-0 mt-[1px] transition',
...
export function TimelineRow({
  ...
  return (
    <div className="flex gap-[14px] fade-in-up cursor-pointer" onClick={() => onOpen(task)}>
      <div className="w-[44px] pt-[2px] text-[0.8125rem] font-semibold text-right shrink-0" style={{ color: task.done ? '#9AA1AA' : '#4C5361' }}>
      ...
      <div className="flex-1 pb-[18px] flex gap-[10px] items-start min-w-0">
        ...
        <div className="flex-1 min-w-0">
          <div className={`text-[0.9375rem] font-medium break-words ${task.done ? 'text-ink-faintest line-through' : 'text-ink'}`}>
